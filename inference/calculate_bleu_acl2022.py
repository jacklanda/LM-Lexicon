...
def bleu(hyp, raw_data, bleu_path="artifact/sentence-bleu", nltk="cpp"):
    assert nltk in [
        "cpp",
        "corpus",
        "sentence",
    ], "nltk param should be cpp/corpus/sentence"
    assert len(hyp) == len(raw_data), "sentence num in hyp not equal to dataset"
    tmp_dir = "/tmp"
...
def main(argv=None):
    ...
    hyp_data = []
    raw_data = []
    with open(gold_src) as fr_src, open(gold_tgt) as fr_tgt, open(hyp_file) as fr_hyp:
    ...
    for src, tgt, hyp in zip(src_content, tgt_content, hyp_content):
        ...
        hyp_data.append(hyp.strip().split(" "))
        raw_data.append((word, exp, tgt.strip()))
