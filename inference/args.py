...
def parse_arguments(desc: str = "Query Large Language Models"):
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Maximum allowed number of tokens for each request.",
    )
    parser.add_argument(
        "--max_tokens",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Minimum allowed number for new generated tokens for each request.",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Number that penalizes the model if it generates the same token after it has been generated in the last 10 tokens.",
    )
    parser.add_argument(
        "--length_penalty",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Generate multiple completions for each prompt and return the best one.",
    )
    parser.add_argument(
        "--verbal",
        action="store_true",
        required=False,
        help="Define whether the model is queried with verbalization.",
    )
    parser.add_argument(
        "--with_vllm",
        action="store_true",
        required=False,
        help="Specify whether the model is deployed with VLLM.",
    )
    parser.add_argument(
        "--run_local_model",
        action="store_true",
        required=False,
        help="Specify whether the model is deployed locally.",
    )
    parser.add_argument(
        "--skip_special_tokens",
        action="store_true",
        required=False,
        help="Do not include special tokens (like the end-of-text token) in the generated text.",
    )
    parser.add_argument(
        "--spaces_between_special_tokens",
        action="store_true",
        required=False,
        help="Add spaces between special tokens (like the end-of-text token) in the generated text.",
    )
    parser.add_argument(
        "--use_beam_search",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="Stop the generation when at least one beam reaches the EOS token.",
    )
    parser.add_argument(
        "--ignore_eos",
        action="store_true",
        required=False,
        help="Ignore the EOS token during generation.",
    )
    parser.add_argument(
        "--top_k",
        type=float,
        default=1.0,
        help="An integer value controlling diversity. 1 means no diversity. 40 seems to be a good value.",
    )
    parser.add_argument(
        "--min_p",
        type=float,
        default=0.0,
        help="An integer value controlling diversity. 0 means no diversity. 0.92 seems to be a good value.",
    )
    parser.add_argument(
        "--evaluate",
        action="store_true",
