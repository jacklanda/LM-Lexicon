...
from calculate_bleu import calc
...
def compute_metric(
    ...
    **kwargs,
    ...
    if metric_name == "bleu":
        # return BLEU_SCORER.compute(predictions=[pred], references=[gold], tokenizer=word_tokenize)["bleu"]  # bleu
        # return BLEU_SCORER.compute(predictions=[pred], references=[gold])["score"] / 100  # sacrebleu
        return BLEU_SCORER(pred, gold)  # sentence-bleu by nltk
        # return BLEU_SCORER.compute(predictions=[pred], references=[gold])["google_bleu"]  # google_bleu
    if metric_name == "meteor":
        return METEOR_SCORER.compute(predictions=[pred], references=[gold])["meteor"]
