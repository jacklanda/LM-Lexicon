...
def tokenize(data_dir, eg_path, ignore_sense_id, one2one):
    ...
    if one2one:
        refs = []
    with open(data_dir, "r", encoding="utf-8") as f1, open(
        eg_path, "r", encoding="utf-8"
    ) as f2:
...
def get_rid_of_period(lst: str):
    pattern = re.compile("\.(?!\d)")
    return [pattern.sub("", sent) for sent in lst]
...
def cal_bleu_for_beams(
    data_dir, type_path, pred_dir, output_dir, beam_sz, mode, one2one, c_range, tmp_dir
):
    ...
    word_desc, references, examples = tokenize(
        ...
        ignore_sense_id=True,
        one2one=one2one,
    )
    test_predictions = read_outputfile(
        word_desc, "{}.forward".format(data_dir + type_path), beam_sz
    )
    ...
    for i in tqdm(
        range(
            int(len(test_predictions) / beam_sz * c_range[0]),
            int(len(test_predictions) / beam_sz * c_range[1]),
        )
    ):
        if one2one:
        ...
        scores += bleu_(
            test_predictions[beam_sz * i : (i + 1) * beam_sz],
            ref_list,
            mode=mode,
            one2one=one2one,
            tmp_dir=tmp_dir,
    ...
    with open(
        "{}_{}_{}{}.txt".format(output_dir, type_path, mode, mose_path),
        "w",
        encoding="utf-8",
    ) as f:
        for i in scores:
